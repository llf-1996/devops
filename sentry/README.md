 ...
 * `config.yml`
 * `sentry.conf.py`
 * `.env.example` — template for environment variables (copied to `.env` during install)
...
├── config.yml              # Sentry main config
├── sentry.conf.py          # Sentry Python config
├── requirements.txt        # Python dependencies
├── Makefile                # Build/push shortcuts (build, push, all)
├── install.sh              # One-click install script
├── test.sh                 # Smoke test script
├── .travis.yml             # CI config (Travis CI)
